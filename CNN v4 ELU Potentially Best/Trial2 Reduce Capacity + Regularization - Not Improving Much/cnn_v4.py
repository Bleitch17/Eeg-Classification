...
class Net(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        # Add stronger batch normalization
        self.bn1 = nn.BatchNorm2d(4, momentum=0.01)  # Reduced from 8
        self.bn2 = nn.BatchNorm2d(16, momentum=0.01)  # Reduced from 40
        
        # Reduced capacity in convolutional layers
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=4, kernel_size=(22, 1), stride=1, padding=0)
        self.conv2 = nn.Conv2d(in_channels=4, out_channels=16, kernel_size=(1, 30), stride=1, padding=0)
        
        # Significantly reduced dense layer capacity
        self.fc0 = nn.LazyLinear(out_features=128)  # Reduced from 400
        self.fc1 = nn.LazyLinear(out_features=64)   # Reduced from 200
        self.fc2 = nn.LazyLinear(out_features=5)
        
        # Much stronger dropout
        self.dropout1 = nn.Dropout(p=0.5)   # Increased from 0.2
        self.dropout2 = nn.Dropout(p=0.5)   # Increased from 0.3
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        # Spatial features with dropout
        x = self.conv1(input)
        x = self.bn1(x)
        x = self.activation(x)
        x = self.spatial_dropout(x)
        
        # Temporal features with dropout
        x = self.conv2(x)
        x = self.bn2(x)
        x = self.activation(x)
        x = self.spatial_dropout(x)
...
if __name__ == "__main__":
    ...
    df = pd.read_parquet("dataset_bci_iv_2a/A01_100.parquet", engine="pyarrow")
