...
class Net(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        x = self.conv1(input)
        x = self.bn1(x)
        x = self.activation(x)  # Using ReLU
...
def plot_training_results(histories):
    ...
    plt.tight_layout()
    plt.savefig('ReLU_training_results.png')
    plt.close()
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(features)):
        ...
        try:
            ...
            if fold_val_acc > best_fold_acc:
                ...
                if os.path.exists('cnn_relu_best.pth'):
                    os.remove('cnn_relu_best.pth')
