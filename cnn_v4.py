...
class Net(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        # Moderate batch normalization
        self.bn1 = nn.BatchNorm2d(8, momentum=0.1)  # Increased from 4
        self.bn2 = nn.BatchNorm2d(32, momentum=0.1)  # Increased from 16
        
        # Moderate capacity in convolutional layers
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=8, kernel_size=(22, 1), stride=1, padding=0)
        self.conv2 = nn.Conv2d(in_channels=8, out_channels=32, kernel_size=(1, 30), stride=1, padding=0)
        
        # Moderate dense layer capacity
        self.fc0 = nn.LazyLinear(out_features=256)  # Increased from 128
        self.fc1 = nn.LazyLinear(out_features=128)  # Increased from 64
        self.fc2 = nn.LazyLinear(out_features=5)
        
        # Moderate dropout
        self.dropout1 = nn.Dropout(p=0.3)   # Reduced from 0.5
        self.dropout2 = nn.Dropout(p=0.3)   # Reduced from 0.5
        
        # Reduced spatial dropout
        self.spatial_dropout = nn.Dropout2d(p=0.1)  # Reduced from 0.2
